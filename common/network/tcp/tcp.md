...
- 疑问？？？
  ...
  - 为啥 wireshark 查看抓包数据时，会出现 3 次挥手 ?
    - tcp 延迟机制导致第二次和第三次合并了（第二次和第三次都是由 server 发送到 client）
...
- TCP VS UDP
  ...
  |C & S| 1 对 1 服务| 多对多服务|
  |可靠性|安全可靠无差|不保证可靠性|
  |流量控制| 控制流量保障传输可靠性| 无控制，网络拥挤也不影响效率|
  |传输方式|流式传输| 一个包一个包传输，可能乱序，丢包|
  |分片方式| 传输层分片，传输层组装，丢包只需传输分片| IP 层分片，组装|
  |应用场景| http/https 、ftp | dns 、广播 包量较少|
